...
export function encryptInAes(source: string, keyStr: string, ivStr: string): string {
...
export function decryptInAes(source: string, keyStr: string, ivStr: string): string {
  const key = enc.Hex.parse(keyStr);
  const iv = enc.Hex.parse(ivStr);
